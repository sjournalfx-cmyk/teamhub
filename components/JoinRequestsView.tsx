...
const JoinRequestsView: React.FC = () => {
    ...
    if (pendingRequests.length === 0) {
        return (
            <div className="p-8 text-center border border-dashed border-black/5 dark:border-white/5 rounded-sm opacity-40">
                <div className="text-[10px] font-black uppercase tracking-widest text-slate-500">No pending invites</div>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-900 dark:text-white">Pending Invites</h3>
            </div>

            <div className="grid gap-3">
                {pendingRequests.map((request) => (
                    <div key={request.id} className="bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 p-4 rounded-sm flex items-center justify-between group hover:border-neon-cyan/30 transition-all">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <div className="text-xs font-black text-slate-900 dark:text-white uppercase tracking-tight">{request.email}</div>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button
                                ...
                                className="flex items-center gap-2 px-4 py-2 bg-neon-cyan text-white dark:text-obsidian-950 text-[9px] font-black uppercase tracking-widest hover:bg-white transition-all"
                            >
