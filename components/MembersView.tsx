...
const MembersView: React.FC = () => {
  ...
  const getActiveTask = (userId: string) => {
    return state.tasks.find(t => 
      (t.assigneeId === userId || (t.assigneeIds && t.assigneeIds.includes(userId))) && 
      t.status === TaskStatus.WorkingOnIt
    );
  };
  ...
  return (
    <div className="flex-1 flex flex-col bg-transparent overflow-y-auto custom-scrollbar p-6 md:p-10 relative">
      ...
      <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <div className="w-10 h-10 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded flex items-center justify-center">
              <Users size={20} className="text-slate-900 dark:text-white" />
            </div>
            <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-wide uppercase">Team Members</h1>
          </div>
          <p className="text-slate-500 text-xs font-bold uppercase tracking-widest">
            List of people and their <span className="text-slate-900 dark:text-white">current tasks</span>.
          </p>
        </div>
        ...
        <div className="flex items-center gap-4">
          {userRole === 'admin' && state.joinRequests.filter(r => r.status === 'pending').length > 0 && (
            <button
              ...
              className={`px-4 py-2.5 rounded-sm border transition-all flex items-center gap-2 ${showRequests ? 'bg-amber-500/20 border-amber-500 text-amber-500' : 'bg-black/5 dark:bg-white/5 border-black/10 dark:border-white/10 text-slate-500 hover:text-slate-900 dark:hover:text-white'}`}
            >
              <AlertCircle size={14} />
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2.5 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-sm text-xs font-medium uppercase tracking-widest text-slate-900 dark:text-white outline-none focus:border-neon-green transition-all w-64"
            />
          </div>
          <button
            onClick={() => setIsInviteModalOpen(true)}
            className="tactical-button px-6 py-2.5 text-slate-900 dark:text-white flex items-center gap-2 group"
          >
            <UserPlus size={16} />
      ...
      <div className="relative z-10 grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
        {[
          { label: 'Team Size', val: state.users.length, color: 'text-slate-900 dark:text-white' },
          ...
          <div key={i} className="bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/5 p-4 rounded-sm">
              ...
              return (
                <tr key={user.id} className="group hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                  <td className="p-4">
                    <div className="flex items-center gap-4">
                      <div className="relative">
                        <img src={user.avatar} className={`w-10 h-10 rounded-sm grayscale group-hover:grayscale-0 transition-all border border-black/10 dark:border-white/10 ${isFocusing ? 'border-neon-green shadow-[0_0_10px_rgba(34,197,94,0.3)]' : ''}`} />
                        <div className={`absolute -bottom-1 -right-1 w-2.5 h-2.5 rounded-full border-2 border-white dark:border-obsidian-950 ${isFocusing ? 'bg-neon-green animate-pulse' : 'bg-slate-400 dark:bg-slate-700'}`}></div>
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                          <div className="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-tight">{user.name}</div>
                  ...
                  <td className="p-4 hidden md:table-cell">
                    <div className="flex items-center text-[16px]">
                      <span className="text-slate-900 dark:text-white font-black w-20">{stats.total}</span>
                  ...
                  <td className="p-4 hidden lg:table-cell">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[10px] font-black text-slate-900 dark:text-white">{stats.progress}%</span>
                    </div>
                  </td>
                  ...
                  <td className="p-4 text-right">
                    <div className="flex justify-end gap-2">
                      {pendingReviews.length > 0 ? (
                        <Tooltip content="Check task work">
                          <button
                        ...
                        <>
                          <button onClick={() => handleEditClick(user)} className="p-2 text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors">
                            <Edit size={14} />
                          </button>
      ...
      {editingUser && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6">
          <div className="absolute inset-0 bg-slate-100/80 dark:bg-obsidian-950/80 backdrop-blur-sm" onClick={() => setEditingUser(null)}></div>
          <div className="relative w-full max-w-md bg-white dark:bg-obsidian-900 border border-black/10 dark:border-white/10 rounded-sm shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-black/5 dark:border-white/5 bg-black/5 dark:bg-white/5 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-widest">Edit Member</h2>
              </div>
              <button onClick={() => setEditingUser(null)} className="text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="p-6 space-y-6">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-sm px-4 py-3 text-xs text-slate-900 dark:text-white outline-none focus:border-neon-green transition-all"
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-sm px-4 py-3 text-xs text-slate-900 dark:text-white outline-none focus:border-neon-green transition-all h-24 resize-none"
                  placeholder="Brief description..."
                />
              </div>
            </div>

            <div className="p-6 bg-black/5 dark:bg-white/5 border-t border-black/5 dark:border-white/5 flex justify-end gap-3">
              <button
              ...
              <button
                ...
                className="tactical-button px-8 py-2.5 text-slate-900 dark:text-white flex items-center gap-2"
              >
                <Check size={14} />
      ...
      <ConfirmModal
        ...
        title="Remove Member"
        message="Are you sure you want to remove this person from the team? This action cannot be undone."
        confirmText="Remove"
        isDestructive={true}
      />
    </div>
