...
const MondayMorningModal: React.FC<MondayMorningModalProps> = ({ isOpen, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-6">
            <div className="absolute inset-0 bg-slate-200/80 dark:bg-obsidian-950/80 backdrop-blur-xl animate-in fade-in duration-500"></div>

            <div className="relative w-full max-w-2xl glass-terminal bg-white dark:bg-slate-900 rounded-sm overflow-hidden animate-in slide-in-from-bottom-12 duration-700 border border-black/10 dark:border-white/10 shadow-[0_0_100px_rgba(0,0,0,0.8)]">

                {/* Tactical Scanning Beam */}
                <div className="absolute top-0 left-0 w-full h-[1px] bg-neon-green/30 animate-scan"></div>

                <div className="p-12 flex flex-col items-center text-center">
                    <div className="w-16 h-16 bg-black/5 dark:bg-white/5 border border-neon-green/30 rounded-full flex items-center justify-center text-neon-green mb-8 shadow-[0_0_20px_rgba(34,197,94,0.2)]">
                        <Sun size={32} />
                    </div>

                    <h2 className="text-2xl font-black text-slate-900 dark:text-white mb-3 uppercase tracking-[0.2em] font-mono">
                    ...
                    <div className="w-full bg-slate-100 dark:bg-black/40 border border-black/5 dark:border-white/5 rounded-sm p-8 mb-10 text-left font-mono">
                        <div className="flex items-center justify-between mb-6 border-b border-black/5 dark:border-white/5 pb-4">
                            ...
                            <div className="flex items-center gap-2">
                                <Calendar size={10} className="text-slate-400 dark:text-slate-700" />
                                <span className="text-[9px] text-slate-400 dark:text-slate-700 tracking-widest uppercase">WEEK_STARTING_{new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric' }).toUpperCase()}</span>
                            </div>
                        </div>
                        ...
                        {displayTasks.length > 0 ? (
                            <div className="space-y-4">
                                {displayTasks.map((task, idx) => (
                                    <div key={task.id} className="flex items-start gap-4 p-4 bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/5 hover:border-neon-green/20 transition-all group">
                                        <span className="text-[10px] font-black text-slate-400 dark:text-slate-700 group-hover:text-neon-green transition-colors">
                                            [0{idx + 1}]
                                        </span>
                                        <div>
                                            <div className="flex items-center gap-2 mb-1">
                                                ...
                                                <span className="text-[8px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest">{formatDate(task.scheduledAt)}</span>
                                            </div>
                                            <p className="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-tight">{task.title}</p>
                                            <div className="flex gap-4 mt-2">
                                                ...
                                                <span className={`text-[8px] px-2 py-0.5 rounded-sm uppercase tracking-widest font-black ${task.priority === Priority.High ? 'bg-rose-500 text-white shadow-[0_0_10px_rgba(244,63,94,0.3)]' : 'bg-neon-cyan/10 text-neon-cyan border border-neon-cyan/20'}`}>{task.priority} Priority</span>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                            ...
                            <div className="text-center py-6 text-slate-400 dark:text-slate-600 text-[10px] uppercase tracking-widest">
                        ...
                        {primaryGoal && (
                            <div className="mt-8 pt-6 border-t border-black/5 dark:border-white/5">
                                <p className="text-[10px] text-slate-500 flex items-center justify-center gap-3 uppercase tracking-widest font-black">
                                    ...
                                    Alignment: <span className="text-slate-900 dark:text-white underline decoration-neon-green/50">{primaryGoal.title}</span>
                                </p>
                            </div>
                        )}
                    </div>
