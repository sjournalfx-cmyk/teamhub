...
const CompletionModal: React.FC<{
  ...
  const addToStage = () => {
    const newItem: Deliverable = {
      ...
      type,
      timestamp: Date.now(),
      url: (type === 'link' || type === 'image' || type === 'csv' || type === 'pdf') ? url : undefined,
                  ...
                  {(type === 'image' || type === 'csv' || type === 'pdf') && (
                    <div className="space-y-3">
                      ...
                      <FileUpload
                        ...
                        accept={type === 'image' ? "image/*" : type === 'csv' ? ".csv" : ".pdf"}
                        label={`Upload ${type.toUpperCase()}`}
                      />
                      {url && (
                        <div className="text-[10px] text-neon-green font-bold uppercase tracking-widest text-center mt-2">
                          {fileName || 'File Ready'}
                        </div>
                      )}
                    </div>
                  ...
                  {type === 'comparison' && (
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <FileUpload
                          onUploadComplete={(url) => setBeforeImg(url)}
                          accept="image/*"
                          label="Before"
                        />
                        {beforeImg && <div className="h-16 border border-white/10 overflow-hidden mt-2"><img src={beforeImg} className="h-full w-full object-cover" /></div>}
                      </div>
                      <div className="space-y-2">
                        <label className="text-[9px] font-black uppercase text-neon-green tracking-widest">After</label>
                        <FileUpload
                          onUploadComplete={(url) => setAfterImg(url)}
                          accept="image/*"
                          label="After"
                        />
                        {afterImg && <div className="h-16 border border-white/10 overflow-hidden mt-2"><img src={afterImg} className="h-full w-full object-cover" /></div>}
                      </div>
                    </div>
                  )}
                </div>
